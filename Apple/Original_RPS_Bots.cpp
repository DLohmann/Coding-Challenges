...
public:
     ...
     */
    virtual void onMatchPlayed(const Bot& botA, const Bot& botB, int winsA, int winsB) = 0;
};
    ...
    void playTournament() {
        ...
        for (size_t i = 0; i < _bots.size(); ++i) {
            for (size_t j = i + 1; j < _bots.size(); ++j) {
                ...
                // Play a match
                pair<int, int> wins = playMatch(*_bots[i], *_bots[j]);

                // Send the results to any observers.
                for (auto* observer : _observers) {
                    observer->onMatchPlayed(*_bots[i], *_bots[j], wins.first, wins.second);
                }
            }
        }
    }
...
private:
     ...
     */
    pair<int, int> playMatch(Bot& playerA, Bot& playerB) {
        pair<int, int> wins;
        playerA.onNewOpponent();
        playerB.onNewOpponent();

        static const int kGamesToPlayPerMatch = gamesPerMatch;
        for (int i = 0; i < kGamesToPlayPerMatch; ++i) {
...
public:
     ...
     */
    void onMatchPlayed(const Bot& botA, const Bot& botB, int winsA, int winsB) override {
        ...
        int total_games = 0;
        for (auto& win : _botWins) {
          total_games += win.second;
        }
        for (auto& loss : _botLosses) {
          total_games += loss.second;
        }
